...
import { ChainId, DEV, CurrencyAmount, Pair, Percent, Route, Token, TokenAmount, Trade, TradeType, WDEV as WETH } from '../src'

describe('Trade', () => {
  const token0 = new Token(ChainId.ROPSTEN, '0x0000000000000000000000000000000000000001', 18, 't0')
  const token1 = new Token(ChainId.ROPSTEN, '0x0000000000000000000000000000000000000002', 18, 't1')
  const token2 = new Token(ChainId.ROPSTEN, '0x0000000000000000000000000000000000000003', 18, 't2')
  const token3 = new Token(ChainId.ROPSTEN, '0x0000000000000000000000000000000000000004', 18, 't3')
  ...
  const pair_weth_0 = new Pair(
    new TokenAmount(WETH[ChainId.ROPSTEN], JSBI.BigInt(1000)),
  ...
  describe('#bestTradeExactIn', () => {
    ...
    it('works for DEV currency input', () => {
      ...
      expect(result[0].route.path).toEqual([WETH[ChainId.ROPSTEN], token0, token1, token3])
      expect(result[0].outputAmount.currency).toEqual(token3)
      expect(result[1].inputAmount.currency).toEqual(DEV)
      expect(result[1].route.path).toEqual([WETH[ChainId.ROPSTEN], token0, token3])
    ...
    it('works for DEV currency output', () => {
      ...
      expect(result[0].route.path).toEqual([token3, token0, WETH[ChainId.ROPSTEN]])
      expect(result[0].outputAmount.currency).toEqual(DEV)
      expect(result[1].inputAmount.currency).toEqual(token3)
      expect(result[1].route.path).toEqual([token3, token1, token0, WETH[ChainId.ROPSTEN]])
